<template>
  <div class="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
    ...
    <div class="max-w-7xl mx-auto mb-12 text-center">
      <h1 class="text-4xl font-bold text-indigo-600 mb-4 sm:text-3xl md:text-4xl">Profil Desa Jatisari</h1>
      <p class="text-lg text-gray-600 max-w-3xl mx-auto text-base sm:text-lg">
        ...
        <div class="border-b border-gray-200">
          <nav class="flex flex-wrap -mb-px justify-center">
            <button
              ...
              :class="[activeTab === tab.id ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300', 'group inline-flex items-center px-4 py-2 md:px-6 md:py-4 border-b-2 font-medium text-sm transition-colors duration-200']"
            >
              <i :class="[tab.icon, 'mr-2 text-sm md:text-base']"></i>
              {{ tab.name }}
            </button>
          </nav>
        </div>
        ...
        <div class="p-4 sm:p-6 md:p-8">
          ...
          <div v-show="activeTab === 'general'" class="space-y-8">
            <div class="bg-indigo-50 rounded-lg p-4 sm:p-6">
              <h2 class="text-2xl font-bold text-indigo-800 mb-4 text-xl sm:text-2xl">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
                <div v-for="(dusun, index) in dusunList" 
                  ...
                  <h3 class="font-semibold text-indigo-600 text-sm sm:text-base">{{ dusun.name }}</h3>
                  <p class="text-gray-600 text-xs sm:text-sm">RW {{ dusun.rw }}: {{ dusun.rt }} RT</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-show="activeTab === 'geographic'" class="space-y-8">
            <div class="bg-green-50 rounded-lg p-4 sm:p-6">
              <h2 class="text-2xl font-bold text-green-800 mb-4 text-xl sm:text-2xl">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                <div class="space-y-4">
                  <div class="bg-white p-4 rounded-lg shadow-sm">
                    <h3 class="font-semibold text-green-600 text-sm sm:text-base mb-2">Koordinat</h3>
                    <p class="text-gray-700 text-xs sm:text-sm">7°83'07.44"S LS dan 111°12'36.52" BT</p>
                  </div>
                  <div class="bg-white p-4 rounded-lg shadow-sm">
                    <h3 class="font-semibold text-green-600 text-sm sm:text-base mb-2">Ketinggian</h3>
                    <p class="text-gray-700 text-xs sm:text-sm">± 450 meter di atas permukaan laut</p>
                  </div>
                  <div class="bg-white p-4 rounded-lg shadow-sm">
                    <h3 class="font-semibold text-green-600 text-sm sm:text-base mb-2">Luas Wilayah</h3>
                    <p class="text-gray-700 text-xs sm:text-sm">260,11 Ha</p>
                  </div>
                </div>
                
                <!-- Border Information -->
                <div class="bg-white p-4 rounded-lg shadow-sm">
                  <h3 class="font-semibold text-green-600 text-sm sm:text-base mb-3">Batas Wilayah</h3>
                  <ul class="space-y-2">
                    <li v-for="(border, index) in borders" 
                        :key="index"
                        class="flex items-center text-gray-700 text-xs sm:text-sm">
                      <span class="w-16 sm:w-20 font-medium">{{ border.direction }}:</span>
          ...
          <div v-show="activeTab === 'demographic'" class="space-y-8">
            <div class="bg-blue-50 rounded-lg p-4 sm:p-6">
              <h2 class="text-2xl font-bold text-blue-800 mb-4 text-xl sm:text-2xl">
                <i class="fas fa-users mr-2"></i>Aspek Demografi
              </h2>
              
              <!-- Occupation Chart -->
              <div class="bg-white p-4 sm:p-6 rounded-lg shadow-sm mb-6">
                <h3 class="font-semibold text-blue-600 text-sm sm:text-base mb-4">Distribusi Mata Pencaharian</h3>
                <div class="relative pt-1">
                  <div v-for="(occupation, index) in occupations" 
                       :key="index"
                       class="mb-4">
                    <div class="flex items-center justify-between mb-1 text-xs sm:text-sm">
                      <span class="text-gray-700">{{ occupation.name }}</span>
                      <span class="text-gray-700">{{ occupation.percentage }}%</span>
                    </div>
          ...
          <div v-show="activeTab === 'social'" class="space-y-8">
            <div class="bg-purple-50 rounded-lg p-4 sm:p-6">
              <h2 class="text-2xl font-bold text-purple-800 mb-4 text-xl sm:text-2xl">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                <div class="bg-white p-4 sm:p-6 rounded-lg shadow-sm">
                  <h3 class="font-semibold text-purple-600 text-sm sm:text-base mb-4">Kelompok Kesenian</h3>
                ...
                <!-- Economic Status -->
                <div class="bg-white p-4 sm:p-6 rounded-lg shadow-sm">
                  <h3 class="font-semibold text-purple-600 text-sm sm:text-base mb-4">Status Ekonomi (DTKS 2022)</h3>
